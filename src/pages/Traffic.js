...
const Traffic = () => {
  ...
  const busLines = () => {
    return data && Array.from(new Set(data.map(item => item.route_short_name)));
  }
  ...
  const FilterByLine = () => {
    ...
    return (
      busLines() &&
      <article className="flex flex-row items-center gap-2 w-1/3">
        <label htmlFor={lineRef}>Filtrar por linea:</label>
        <select ref={lineRef} name="lines" onChange={() => displayValue()} className="cursor-pointer">
      ...
      ? <>
        ...
        <Menu />
        {
          isLoading
            ...
            : <main className="flex flex-col p-2 gap-2 max-sm:mt-32 sm:mt-12 h-[90vh]">
              <FilterByLine />
